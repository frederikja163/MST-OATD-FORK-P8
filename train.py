...
def main():
    ...
    if args.task == 'train':
        ...
        for epoch in range(args.pretrain_epochs):
            MST_OATD.pretrain(train_loader, epoch)

        MST_OATD.train_gmm(train_loader)
    ...
    if args.task == 'test':
        test_trajs = np.load(
            f'./data/{args.dataset}/outliers_data_init_{args.distance}_{args.fraction}_{args.obeserved_ratio}.npy',
            allow_pickle=True)
        outliers_idx = np.load(
            f"./data/{args.dataset}/outliers_idx_init_{args.distance}_{args.fraction}_{args.obeserved_ratio}.npy",
            allow_pickle=True)
        ...
        pr_auc = MST_OATD.detection(outliers_loader, labels)
        pr_auc = "%.4f" % pr_auc
